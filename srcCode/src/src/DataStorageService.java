...
public class DataStorageService extends DataStorageAPIImplBase {
	...
	public void read(FileInput request, StreamObserver<IntegerIterator> responseObserver) {
		...
		if (response != null) {
			while (response.hasNext()) {
				responseObserver.onNext(IntegerIterator.newBuilder().
						addValue(response.next()).build());
			}
	...
	@Override
	public void userTranslate(FileOutput request, 
			StreamObserver<WritingResult> responseObserver) {
